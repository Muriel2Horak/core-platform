 ...
 * 
 * Endpoints for workflow simulation and testing:
 * - Simulate workflow execution (dry-run)
 * - Generate test scenarios
 * - Validate transitions without side effects
 * 
 * @since 2025-01-14
 */
@RestController
@RequestMapping("/api/v1/workflows/testing")
...
public class WorkflowTestingController {
    ...
    @Timed(value = "workflow.testing.simulate", description = "Time to simulate workflow")
    public ResponseEntity<SimulationResult> simulate(@RequestBody SimulationRequest request) {
        log.info("Simulating workflow: entity_type={}, scenario={}", 
                request.entityType(), request.scenarioName());
     ...
     */
    @GetMapping("/scenarios/{entityType}/{scenarioType}")
    @Timed(value = "workflow.testing.generate_scenario", description = "Time to generate test scenario")
    public ResponseEntity<TestScenario> generateScenario(
     ...
     */
    @PostMapping("/suites/{entityType}/run")
    @PreAuthorize("hasRole('WORKFLOW_ADMIN')")
    @Timed(value = "workflow.testing.run_suite", description = "Time to run test suite")
    public ResponseEntity<TestSuiteResult> runTestSuite(@PathVariable String entityType) {
        ...
        int passed = (result1.success() ? 1 : 0) + (result2.success() ? 1 : 0) + (result3.success() ? 1 : 0);
        int total = 3;
