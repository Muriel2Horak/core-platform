 ...
 * 
 * Manages workflow schema versions and migrations: - Create new versions -
 * Activate/deactivate versions - Migrate instances between versions - Version
...
public class WorkflowVersionService {
     ...
     */
    public Long createVersion(String entityType, JsonNode schemaDefinition, String createdBy,
            String notes) {
    ...
    public void activateVersion(Long versionId) {
        ...
        String entityType = jdbcTemplate.queryForObject(
                "SELECT entity_type FROM workflow_versions WHERE id = ?", String.class, versionId);

        // Deactivate all other versions for this entity
        jdbcTemplate.update("UPDATE workflow_versions SET is_active = false WHERE entity_type = ?",
                entityType);

        // Activate this version
        jdbcTemplate.update("UPDATE workflow_versions SET is_active = true WHERE id = ?",
                versionId);
    ...
    public Optional<WorkflowVersion> getActiveVersion(String entityType) {
        List<WorkflowVersion> versions = jdbcTemplate.query(
                """
                        ...
                        """,
                (rs, rowNum) -> new WorkflowVersion(rs.getLong("id"), rs.getString("entity_type"),
                        rs.getInt("version"), rs.getString("schema_definition"),
                        rs.getString("created_by"), rs.getTimestamp("created_at").toInstant(),
                        rs.getBoolean("is_active"), rs.getString("migration_notes")),
                entityType);
     ...
     */
    public Long startMigration(Long fromVersionId, Long toVersionId, MigrationStrategy strategy,
            String createdBy) {
        Long migrationId = jdbcTemplate.queryForObject("""
                ...
                VALUES (?, ?, ?, ?, ?)
                RETURNING id
                """, Long.class, fromVersionId, toVersionId, strategy.name(), createdBy,
                Timestamp.from(Instant.now()));
    ...
    public List<WorkflowMigration> getMigrationHistory(Long instanceId) {
        return jdbcTemplate.query("""
                ...
                """,
                (rs, rowNum) -> new WorkflowMigration(rs.getLong("id"),
                        rs.getLong("workflow_instance_id"), rs.getLong("from_version_id"),
