 ...
 * 
 * Simulation mode pro testování workflow bez side effects:
 * - Dry-run execution (žádné DB zápisy)
...
public class WorkflowTestingService {
    ...
    public SimulationResult simulate(SimulationRequest request) {
        log.info("Starting workflow simulation: entity_type={}, scenario={}", 
        ...
        for (TransitionEvent event : request.events()) {
            SimulationStep step = executeTransition(
                    request.entityType(),
                    currentState,
                    event,
                    request.mockData()
    ...
    private SimulationStep executeTransition(
            String entityType,
            String fromState,
        ...
        if (targetState == null) {
            return new SimulationStep(
                    ...
                    event.name(),
                    false,
                    "No transition found for event: " + event.name(),
                    null,
                    null
            );
        }
        ...
        if (!guardsPassed) {
            String failedGuards = guardResults.stream()
                    .filter(g -> !g.passed())
                    .map(GuardResult::guardName)
                    .reduce((a, b) -> a + ", " + b)
                    .orElse("");
    ...
    private List<GuardResult> evaluateGuards(List<String> guards, Map<String, Object> mockData) {
        return guards.stream()
                .map(guard -> evaluateGuard(guard, mockData))
                .toList();
    }
    ...
    private List<ActionResult> simulateActions(List<String> actions, Map<String, Object> mockData) {
        return actions.stream()
                .map(action -> simulateAction(action, mockData))
                .toList();
    }
    ...
    private TestScenario generateHappyPath(String entityType) {
        ...
        return new TestScenario(
                ...
                "START",
                List.of(
                        new TransitionEvent("create", Map.of()),
    ...
    private TestScenario generateErrorCases(String entityType) {
        return new TestScenario(
                ...
                "START",
                List.of(
                        new TransitionEvent("submit", Map.of()), // Error: missing userId
