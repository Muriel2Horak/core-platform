 ...
 * 
 * Orchestrates executor invocation with:
 * - Async execution
...
public class WorkflowExecutionService {
     ...
     */
    private CompletableFuture<Map<String, Object>> executeWithRetry(
        ...
        return executeAttempt(executor, entityType, entityId, actionCode, context, 1, policy)
            .exceptionally(ex -> {
                log.error("Executor {} failed after {} attempts: {}", 
                    executor.getName(), policy.maxAttempts(), ex.getMessage());
     ...
     */
    private CompletableFuture<Map<String, Object>> executeAttempt(
        WorkflowExecutor executor,
        String entityType,
        String entityId,
        String actionCode,
        Map<String, Object> context,
        int attempt,
        WorkflowExecutor.RetryPolicy policy
    ) {
        ...
        return executor.execute(entityType, entityId, actionCode, context)
            .whenComplete((result, ex) -> {
                ...
                if (ex == null) {
                    ...
                    log.info("Executor {} completed for {}/{} in {}ms", 
                        executor.getName(), entityType, entityId, duration);
                }
            })
            .exceptionally(ex -> {
                if (attempt < policy.maxAttempts()) {
                    ...
                    log.warn("Executor {} attempt {}/{} failed, retrying in {}ms: {}", 
                        executor.getName(), attempt, policy.maxAttempts(), delay, ex.getMessage());
                    ...
                    return executeAttempt(executor, entityType, entityId, actionCode, context, attempt + 1, policy).join();
                }
     ...
     */
    public CompletableFuture<Void> compensateAction(
        String entityType,
        String entityId,
        String actionCode,
        ...
        var futures = executors.stream()
            .map(executor -> executor.compensate(entityType, entityId, actionCode, originalContext)
                .exceptionally(ex -> {
                    log.error("Compensation failed for executor {}: {}", 
                        executor.getName(), ex.getMessage());
                    return null;
                }))
            .toArray(CompletableFuture[]::new);
    ...
    private long calculateBackoff(int attempt, WorkflowExecutor.RetryPolicy policy) {
        long delay = (long) (policy.initialDelayMs() * Math.pow(policy.backoffMultiplier(), attempt - 1));
