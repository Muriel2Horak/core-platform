 ...
 * 
 * Updates inventory levels on order workflow transitions.
 * Example: Order confirmed → Reserve inventory
 * 
 * Demonstrates:
 * - Critical business logic execution
 * - Compensation with inventory rollback
 * - Custom retry policy (no retry for idempotent updates)
 * 
 * @since 2025-01-14
 */
@Component
public class UpdateInventoryExecutor implements WorkflowExecutor {
    ...
    public CompletableFuture<Map<String, Object>> execute(
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            log.info("Updating inventory: product={}, quantity={}, operation={}", 
    ...
    public CompletableFuture<Void> compensate(
        ...
        return CompletableFuture.runAsync(() -> {
            ...
            log.warn("Compensating inventory update: product={}, rollback {}→{}", 
                productId, newStock, previousStock);
