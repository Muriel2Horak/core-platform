 ...
 * 
 * Central registry for all workflow executors.
 * Supports dynamic executor registration and lookup.
 * 
 * @since 2025-01-14
 */
@Component
public class WorkflowExecutorRegistry {
    ...
    /**
     * Register an executor (called automatically by Spring for @Component executors)
     */
    ...
    public List<WorkflowExecutor> findExecutorsForAction(String actionCode) {
        return executorsByAction.computeIfAbsent(actionCode, action -> 
            executorsByName.values().stream()
                .filter(ex -> ex.supports(action))
                .toList()
        );
    }
