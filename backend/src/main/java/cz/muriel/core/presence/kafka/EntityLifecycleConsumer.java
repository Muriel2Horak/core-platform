 ...
 * 
 * Responsibilities:
 * - On MUTATING: Mark entity as "stale" in Redis (prevent concurrent edits)
 * - On MUTATED: Clear "stale" flag, increment version in Redis
 * - Broadcast updates via WebSocket (handled by notification service)
 * 
 * Retry Strategy:
...
public class EntityLifecycleConsumer {
  ...
  @KafkaListener(
      topics = "core.entities.lifecycle.mutating",
      groupId = "presence-service",
      containerFactory = "kafkaListenerContainerFactory"
  )
  public void consumeMutating(
  ...
  public void consumeMutated(
    ...
    try {
      ...
      log.info("Processed MUTATED event: {}:{} version {} by {} (offset: {})", 
          entity, id, version, userId, offset);
  ...
  @KafkaListener(
      topics = {
          "core.entities.lifecycle.mutating.dlt",
          "core.entities.lifecycle.mutated.dlt"
      },
      groupId = "presence-service-dlq"
  )
