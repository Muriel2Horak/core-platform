 ...
 * 
 * Validates: - Maximum rows limit - Maximum time interval - Required time range
 */
...
public class QueryRequestValidator implements ConstraintValidator<ValidQuery, QueryRequest> {
    ...
    if (query.getTimeRange() != null) {
      ...
      if (start != null && end != null) {
        if (end.isBefore(start)) {
          context.buildConstraintViolationWithTemplate("Time range end must be after start")
              .addPropertyNode("timeRange.end").addConstraintViolation();
          isValid = false;
        }
  ...
  private boolean isEntityExemptFromTimeRange(String entity) {
    ...
    return entity != null && (entity.equals("entity_types") || entity.equals("field_types")
