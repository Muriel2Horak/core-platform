 ...
 * 
 * <p>Admin-only endpoints for generating Cube.js schemas.
 */
@Slf4j
@RestController
@RequestMapping("/api/admin/cube/modelgen")
...
public class CubeModelgenController {
  ...
  public ResponseEntity<Map<String, Object>> exportEntity(@PathVariable String entityName) {
    ...
    try {
      ...
      return ResponseEntity.ok(Map.of(
          "message", "Cube.js schema exported successfully",
          "entity", entityName,
          "file", file.toString()
      ));
    } catch (Exception e) {
  ...
  public ResponseEntity<Map<String, Object>> status() {
    return ResponseEntity.ok(Map.of(
        "autoExportEnabled", modelgenService.isAutoExportEnabled(),
