 ...
 * 
 * The fingerprint includes: - Tenant ID - Entity name - Sorted dimensions -
 * Sorted measures - Sorted filters - Time range - Metamodel spec version (for
 * cache invalidation)
 */
    ...
    try {
      ...
      if (query.getMeasures() != null && !query.getMeasures().isEmpty()) {
        List<String> sortedMeasures = query.getMeasures().stream()
            ...
            .map(m -> m.getField() + ":" + m.getAggregation()).collect(Collectors.toList());
      ...
      if (query.getOrderBy() != null && !query.getOrderBy().isEmpty()) {
        List<String> orderBy = query.getOrderBy().stream()
            .map(o -> o.getField() + ":" + o.getDirection()).collect(Collectors.toList());
