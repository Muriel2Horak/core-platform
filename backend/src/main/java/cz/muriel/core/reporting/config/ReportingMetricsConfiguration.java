...
/**
 * Prometheus metrics for reporting module. Exposes metrics for Grafana
 * dashboard.
 */
...
public class ReportingMetricsConfiguration {
  ...
  public void registerMetrics() {
    ...
    Gauge
        .builder("reporting.query.inflight", queryDeduplicator, QueryDeduplicator::getInflightCount)
        .description("Number of in-flight deduplicated queries").register(meterRegistry);
    ...
    circuitBreakerRegistry.getAllCircuitBreakers().forEach(circuitBreaker -> {
      ...
      Gauge
          .builder("reporting.circuit_breaker.failure_rate", circuitBreaker,
              cb -> cb.getMetrics().getFailureRate())
          .tag("circuit_breaker", cbName).description("Circuit Breaker failure rate (%)")
          .register(meterRegistry);

      Gauge
          .builder("reporting.circuit_breaker.slow_call_rate", circuitBreaker,
              cb -> cb.getMetrics().getSlowCallRate())
          .tag("circuit_breaker", cbName).description("Circuit Breaker slow call rate (%)")
          .register(meterRegistry);
    });
  }
}
