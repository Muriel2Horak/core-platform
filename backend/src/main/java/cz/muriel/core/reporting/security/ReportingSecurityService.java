...
public class ReportingSecurityService {
  ...
  private static final Set<String> ADMIN_ONLY_ENTITIES = Set.of("AuditLog", "SystemConfig",
      "TenantSettings");

  // Sensitive fields that require elevated permissions
  private static final Set<String> SENSITIVE_FIELDS = Set.of("password", "api_key", "secret",
   ...
   */
  public void validateFieldAccess(List<String> dimensions, List<String> measures,
  ...
  public void validateQueryComplexity(int dimensionCount, int measureCount, int filterCount) {
    ...
    if (dimensionCount > MAX_DIMENSIONS) {
      throw new IllegalArgumentException(
          "Too many dimensions: " + dimensionCount + " (max: " + MAX_DIMENSIONS + ")");
    }

    if (measureCount > MAX_MEASURES) {
      throw new IllegalArgumentException(
          "Too many measures: " + measureCount + " (max: " + MAX_MEASURES + ")");
    }

    if (filterCount > MAX_FILTERS) {
      throw new IllegalArgumentException(
          "Too many filters: " + filterCount + " (max: " + MAX_FILTERS + ")");
    }
  }
}
