 ...
 * 
 * Protects backend from Cube.js failures:
 * - Opens circuit after 50% failure rate in 10 requests
 * - Half-open after 30 seconds
 * - Fallback: Return cached data or error response
 * 
 * Monitoring:
 * - Metrics exposed via /actuator/metrics/resilience4j.circuitbreaker
 * - Grafana dashboard for CB state changes
 */
@Slf4j
@Configuration
public class CircuitBreakerConfiguration {
    ...
    public CircuitBreaker cubeQueryCircuitBreaker(CircuitBreakerRegistry circuitBreakerRegistry) {
        ...
        circuitBreaker.getEventPublisher()
            .onStateTransition(event -> 
                log.warn("Cube Circuit Breaker state changed: {} -> {}",
                    event.getStateTransition().getFromState(),
                    event.getStateTransition().getToState()))
            .onError(event -> 
                log.error("Cube query failed: {}", event.getThrowable().getMessage()))
            .onSuccess(event -> 
                log.debug("Cube query succeeded in {}ms", 
                    event.getElapsedDuration().toMillis()))
            .onCallNotPermitted(event -> 
                log.warn("Cube query rejected: Circuit Breaker is OPEN"));
    ...
    public CircuitBreaker cubeMetaCircuitBreaker(CircuitBreakerRegistry circuitBreakerRegistry) {
        ...
        CircuitBreakerConfig config = CircuitBreakerConfig.custom()
            .failureRateThreshold(70) // 70% failure rate
