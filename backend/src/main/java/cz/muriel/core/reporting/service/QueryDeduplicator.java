 ...
 * query is executed against Cube.js. Other requests wait for the result
 * and share the same response.
 * 
...
public class QueryDeduplicator {
    ...
    /**
     * Executes a query with deduplication.
     * If an identical query is already in flight, waits for its result.
     * Otherwise, executes the query and shares the result with waiters.
     *
     ...
     * @return Query result
     */
    public Map<String, Object> executeWithDeduplication(
        ...
        CompletableFuture<Map<String, Object>> future = inflightQueries.computeIfAbsent(fingerprint, key -> {
        ...
        try {
            if (inflightQueries.containsKey(fingerprint) && future != inflightQueries.get(fingerprint)) {
                log.info("Deduplicating query - waiting for in-flight result (fingerprint: {})", fingerprint);
            }
    ...
    /**
     * Computes SHA-256 fingerprint of query + tenant for deduplication.
     * Identical queries (same dimensions, filters, tenant) get same fingerprint.
     *
     * @param query Cube.js query object
    ...
    /**
     * Returns number of currently in-flight queries.
    ...
    /**
     * Clears all in-flight queries.
     * Used for testing and forced cache invalidation.
     */
    public void clear() {
