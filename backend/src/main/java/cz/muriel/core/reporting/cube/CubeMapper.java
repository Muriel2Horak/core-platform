...
public class CubeMapper {
  ...
  public CubeQueryRequest toCubeQuery(QueryRequest query, String tenantId) {
    ...
    if (query.getMeasures() != null && !query.getMeasures().isEmpty()) {
      List<String> measures = query.getMeasures().stream()
          .map(m -> toeCubeMeasure(query.getEntity(), m)).collect(Collectors.toList());
    ...
    if (query.getDimensions() != null && !query.getDimensions().isEmpty()) {
      List<String> dimensions = query.getDimensions().stream()
          .map(d -> toCubeDimension(query.getEntity(), d)).collect(Collectors.toList());
    ...
    if (query.getTimeRange() != null) {
      ...
      CubeQueryRequest.TimeDimension timeDim = CubeQueryRequest.TimeDimension.builder()
          ...
          .dateRange(List.of(query.getTimeRange().getStart().toString(),
    ...
    cubeFilters.add(
        CubeQueryRequest.Filter.builder().member(toCubeDimension(query.getEntity(), "tenant_id"))
    ...
    if (query.getFilters() != null && !query.getFilters().isEmpty()) {
      for (QueryRequest.Filter filter : query.getFilters()) {
        CubeQueryRequest.Filter cubeFilter = CubeQueryRequest.Filter.builder()
            ...
            .operator(mapOperator(filter.getOperator())).values(normalizeValue(filter.getValue()))
            .build();
    ...
    if (query.getOrderBy() != null && !query.getOrderBy().isEmpty()) {
      List<List<String>> order = query.getOrderBy().stream().map(o -> List
          .of(toCubeDimension(query.getEntity(), o.getField()), o.getDirection().toLowerCase()))
  ...
  private String mapOperator(String dslOperator) {
    ...
    case "lte":
      return "lte";
    case "in":
      return "equals"; // Cube uses equals with array
    case "notin":
      return "notEquals";
    case "contains":
      return "contains";
