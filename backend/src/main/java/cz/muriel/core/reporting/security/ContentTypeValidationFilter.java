 ...
 * 
 * Enforces strict Content-Type validation for POST/PUT/PATCH requests: - Only
 * application/json allowed for API endpoints - Prevents content-type confusion
 * attacks - Returns 415 Unsupported Media Type if invalid
 * 
 * OWASP Compliance: - A3:2021 - Injection (prevents XML/file upload attacks) -
...
public class ContentTypeValidationFilter extends OncePerRequestFilter {
  ...
  protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response,
    ...
    if ((path.startsWith("/api/reports") || path.startsWith("/api/entities")
        || path.startsWith("/api/bulk-jobs")) && METHODS_WITH_BODY.contains(method)) {
      ...
      if (contentType == null || contentType.isEmpty()) {
        ...
        sendError(response, HttpServletResponse.SC_BAD_REQUEST, "Content-Type header is required");
        return;
      }
      ...
      if (!baseContentType.equalsIgnoreCase(ALLOWED_CONTENT_TYPE)) {
        log.warn("Invalid Content-Type: {} for {} {} (expected: {})", baseContentType, method, path,
  ...
  private void sendError(HttpServletResponse response, int status, String message)
    ...
    String errorJson = String.format("{\"error\":\"%s\",\"message\":\"%s\"}", getStatusText(status),
        message);
