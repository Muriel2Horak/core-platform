...
/**
 * Service for executing Cube.js queries with Circuit Breaker protection.
 * Wraps Cube.js HTTP API with resilience patterns.
 */
@Service
@Slf4j
...
public class CubeQueryService {
     ...
     * Circuit Breaker is per-tenant to isolate failures.
     *
    ...
    /**
     * Fetches entity metadata (schema) from Cube.js.
     * Uses separate Circuit Breaker with more lenient threshold (70%).
     *
    ...
    public Map<String, Object> getEntityMetadata(String entityName, String tenantId) {
        CircuitBreaker circuitBreaker = circuitBreakerRegistry.circuitBreaker(
            "cubeMetadataCircuitBreaker-" + tenantId
        );

        return circuitBreaker.executeSupplier(() -> {
            log.debug("Fetching metadata for entity {} (tenant {})", entityName, tenantId);
            
            return cubeWebClient.get()
                .uri("/cubejs-api/v1/meta")
