 ...
 */
@RestController @RequestMapping("/api/documents") @RequiredArgsConstructor @Slf4j
public class DocumentController {
  ...
  public ResponseEntity<Map<String, Object>> uploadDocument(
      @RequestParam("file") MultipartFile file, @RequestParam(required = false) String entityType,
      @RequestParam(required = false) String entityId, Authentication auth) {
    try {
      String tenantId = TenantContext.getTenantKey();

      DocumentModels.UploadResult result = documentService.uploadDocument(auth, tenantId,
          entityType, entityId, file);

      return ResponseEntity
          .ok(Map.of("success", true, "documentId", result.getDocumentId().toString(),
              "downloadUrl", result.getDownloadUrl(), "message", result.getMessage()));
  ...
  public ResponseEntity<DocumentModels.Document> getDocument(@PathVariable UUID documentId) {
    try {
  ...
  public ResponseEntity<List<DocumentModels.Document>> listDocuments(
    ...
    try {
      String tenantId = TenantContext.getTenantKey();
      List<DocumentModels.Document> docs = documentService.listDocuments(tenantId, entityType,
          entityId);
