 ...
 * 
 * RBAC:
 * - GET /api/admin/ai/config: PLATFORM_ADMIN, OPS, TENANT_ADMIN (read-only)
 * - PUT /api/admin/ai/config: PLATFORM_ADMIN, OPS only
 * 
 * Features:
...
public class AdminAiConfigController {
     ...
     * Get global AI configuration
     * 
     * Accessible by: PLATFORM_ADMIN, OPS, TENANT_ADMIN
     * TenantAdmin gets read-only view
     */
    @GetMapping("/config")
    @PreAuthorize("hasAnyRole('PLATFORM_ADMIN', 'OPS', 'TENANT_ADMIN')")
    public ResponseEntity<GlobalAiConfig> getAiConfig() {
        ...
        try {
            ...
            log.info("✅ AI config fetched: enabled={}, mode={}", 
     ...
     * 
     * NOTE: In this version, this endpoint only validates the config.
     * Actual persistence to global-config.yaml is not implemented yet.
     * Future versions will:
     * 1. Persist to global-config.yaml
     * 2. Trigger hot reload of metamodel
     * 3. Publish config change event to Kafka
     */
    @PutMapping("/config")
    ...
    public ResponseEntity<Map<String, String>> updateAiConfig(@RequestBody GlobalAiConfig aiConfig) {
        ...
        try {
            ...
            return ResponseEntity.ok(Map.of(
                    "status", "validated",
                    "message", "AI config validated successfully. Persistence not implemented yet.",
                    "note", "Future versions will persist to global-config.yaml and trigger hot reload"
            ));
    ...
    public ResponseEntity<Map<String, Object>> getAiStatus() {
        ...
        try {
            ...
            String mode = aiConfig != null && aiConfig.getMode() != null 
                    ? aiConfig.getMode().toString() 
                    : "META_ONLY";
