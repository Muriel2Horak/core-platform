...
@Slf4j
@RestController
@RequestMapping("/api/admin/grafana")
...
public class GrafanaSyncController {
    ...
    public ResponseEntity<Map<String, Object>> syncAllUsers(@RequestParam(required = false) String realm) {
        log.info("🔄 Manual Grafana sync triggered for realm: {}", realm != null ? realm : "admin");
        
        try {
            Map<String, Object> result = grafanaUserSyncService.syncAllMonitoringUsers(
                    realm != null ? realm : "admin"
            );
