...
/**
 * Service for proxying requests to Grafana HTTP API.
 * Adds service account token and X-Grafana-Org-Id header.
...
public class MonitoringProxyService {
    ...
    /**
     * Forward POST /api/ds/query request to Grafana.
     * Used by Grafana Scenes to query datasources.
     */
    public ResponseEntity<String> forwardQuery(Jwt jwt, Map<String, Object> body) {
        ...
        try {
            String response = grafanaClient.post()
                .uri("/api/ds/query")
                .header(HttpHeaders.AUTHORIZATION, "Bearer " + binding.serviceAccountToken())
                .header("X-Grafana-Org-Id", String.valueOf(binding.orgId()))
                .bodyValue(body)
                .retrieve()
            ...
            return ResponseEntity.internalServerError()
                .body("{\"error\":\"Failed to query Grafana\"}");
        }
    }
