...
/**
 * Default implementation of TenantOrgService.
 * Loads tenant-to-org mapping from environment/config.
 * In production, this should load from Vault/Secrets Manager.
 */
@Service
@Slf4j
public class TenantOrgServiceImpl implements TenantOrgService {
    ...
    public void init() {
        ...
        tenantOrgMap.put("core-platform", new TenantBinding(
            "core-platform",
            1L,
            System.getenv().getOrDefault("GRAFANA_SAT_CORE_PLATFORM", "glsa_dev_token_placeholder")
        ));
        
        tenantOrgMap.put("test-tenant", new TenantBinding(
            "test-tenant",
            2L,
