...
@Slf4j
@Tag(name = "Monitoring BFF", description = "Backend-for-Frontend proxy for Grafana with tenant isolation")
...
public class MonitoringProxyController {
  ...
  @Operation(
      ...
      description = "Proxies datasource queries to Grafana with automatic tenant isolation. " +
          "Adds service account token and X-Grafana-Org-Id header based on JWT tenant claim."
  )
  @ApiResponses({
      @ApiResponse(
          responseCode = "200", 
          ...
          content = @Content(
              ...
              examples = @ExampleObject(value = "{\"results\":{\"A\":{\"frames\":[...]}}}")
          )
      ),
      @ApiResponse(
          responseCode = "401", 
      ...
      @ApiResponse(
          responseCode = "500", 
          description = "Internal server error or Grafana unavailable"
      ),
      @ApiResponse(
          responseCode = "503", 
          description = "Circuit breaker open - Grafana service unavailable"
      )
  })
  public ResponseEntity<String> query(
      @Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt,
      @io.swagger.v3.oas.annotations.parameters.RequestBody(
          description = "Grafana datasource query request",
          content = @Content(
              mediaType = "application/json",
              examples = @ExampleObject(
                  name = "Prometheus query example",
                  value = "{\"queries\":[{\"refId\":\"A\",\"expr\":\"up\",\"range\":true,\"instant\":false}],\"from\":\"now-1h\",\"to\":\"now\"}"
              )
          )
      )
  ...
  @Operation(
      ...
      description = "Returns available datasources for the tenant's Grafana organization. " +
          "Response contains minimal metadata (uid, name, type) - sensitive data like credentials are filtered."
  )
  @ApiResponses({
      @ApiResponse(
          responseCode = "200", 
          ...
          content = @Content(
              mediaType = "application/json",
              examples = @ExampleObject(
                  value = "[{\"uid\":\"prometheus\",\"name\":\"Prometheus\",\"type\":\"prometheus\"}]"
              )
          )
      ),
  ...
  public ResponseEntity<String> datasources(@Parameter(hidden = true) @AuthenticationPrincipal Jwt jwt) {
  ...
  @Operation(
      ...
      description = "Returns dashboard definition for embedding in Grafana Scenes. " +
          "Dashboard must exist in the tenant's Grafana organization."
  )
  @ApiResponses({
      @ApiResponse(
          responseCode = "200", 
  ...
  @Operation(
      ...
      description = "Verifies that Grafana service is reachable and responsive. " +
          "Returns Grafana's health status without requiring authentication."
  )
  @ApiResponses({
      @ApiResponse(
          responseCode = "200", 
          ...
          content = @Content(
              mediaType = "application/json",
              examples = @ExampleObject(value = "{\"database\":\"ok\"}")
          )
      ),
      @ApiResponse(
          responseCode = "503", 
          ...
          content = @Content(
              mediaType = "application/json",
              examples = @ExampleObject(
                  value = "{\"status\":\"error\",\"message\":\"Grafana unavailable\"}"
              )
          )
      )
  })
  ...
  @RequestMapping(value = "/**", method = { RequestMethod.PUT, RequestMethod.DELETE,
      RequestMethod.PATCH })
  @Operation(
      summary = "Block mutations",
      description = "Security endpoint that blocks all PUT/DELETE/PATCH requests to prevent " +
          "unauthorized modifications to Grafana configuration via BFF."
  )
  @ApiResponse(
      responseCode = "403", 
      description = "Mutation not allowed",
      content = @Content(
          mediaType = "application/json",
          examples = @ExampleObject(value = "{\"error\":\"Mutations not allowed via BFF\"}")
      )
  )
