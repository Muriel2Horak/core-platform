...
/**
 * Rate limiting filter for monitoring BFF endpoints. Limits requests per user
 * (based on JWT subject) to prevent abuse.
 */
@Component @Slf4j
public class MonitoringRateLimitFilter extends OncePerRequestFilter {
  ...
  private Bucket createNewBucket() {
    Bandwidth limit = Bandwidth.builder().capacity(CAPACITY)
        .refillIntervally(REFILL_TOKENS, REFILL_DURATION).build();
