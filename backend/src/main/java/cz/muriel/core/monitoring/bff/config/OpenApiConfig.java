...
@OpenAPIDefinition(
    info = @Info(
        title = "Core Platform - Monitoring BFF API",
        version = "1.0.0",
        description = """
        ...
        contact = @Contact(
            name = "Platform Team",
            email = "platform@core-platform.local"
        ),
        license = @License(
            name = "Proprietary",
            url = "https://core-platform.local/license"
        )
    ),
    servers = {
        @Server(
            url = "https://app.core-platform.local",
...
@SecurityScheme(
    ...
    scheme = "bearer",
    bearerFormat = "JWT",
    description = "JWT token obtained from Keycloak OAuth2 flow. " +
