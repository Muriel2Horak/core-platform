...
public interface UserDirectoryRepository extends JpaRepository<UserDirectoryEntity, UUID> {
        ...
        // 📊 TENANT STATISTICS: Počítání uživatelů podle tenant id
        long countByTenantId(UUID tenantId);

        // Základní vyhledávání podle tenant id
        List<UserDirectoryEntity> findByTenantId(UUID tenantId);

        Optional<UserDirectoryEntity> findByTenantIdAndUsername(UUID tenantId, String username);

        Optional<UserDirectoryEntity> findByTenantIdAndEmail(UUID tenantId, String email);

        Optional<UserDirectoryEntity> findByTenantIdAndKeycloakUserId(UUID tenantId,
        ...
        List<UserDirectoryEntity> findByUsernameOrEmailIgnoreCase(
                        @Param("identifier") String username, @Param("identifier") String email);

        // Vyhledávání s wildcards
        @Query("SELECT u FROM UserDirectoryEntity u WHERE u.tenantId = :tenantId AND (LOWER(u.username) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR LOWER(u.email) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR LOWER(u.firstName) LIKE LOWER(CONCAT('%', :searchTerm, '%')) OR LOWER(u.lastName) LIKE LOWER(CONCAT('%', :searchTerm, '%')))")
        List<UserDirectoryEntity> searchByTenantIdAndTerm(@Param("tenantId") UUID tenantId,
