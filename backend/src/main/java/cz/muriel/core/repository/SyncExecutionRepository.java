...
public interface SyncExecutionRepository extends JpaRepository<SyncExecution, String> {
    ...
    Page<SyncExecution> findByStatusAndTenantKey(SyncStatus status, String tenantKey, Pageable pageable);
    
    @Query("SELECT s FROM SyncExecution s WHERE s.status = :status AND s.startTime >= :since ORDER BY s.startTime DESC")
    List<SyncExecution> findRecentByStatus(@Param("status") SyncStatus status, @Param("since") LocalDateTime since);
    
    @Query("SELECT s FROM SyncExecution s WHERE s.tenantKey = :tenantKey ORDER BY s.startTime DESC")
    Page<SyncExecution> findByTenantKeyOrderByStartTimeDesc(@Param("tenantKey") String tenantKey, Pageable pageable);
