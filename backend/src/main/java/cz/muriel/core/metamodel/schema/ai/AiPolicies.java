 ...
 * 
 * Controls what data can be exposed to AI agents. Default mode is META_ONLY for
 * maximum security.
 * 
 * @since 2025-10-14
 */
...
public class AiPolicies {
  ...
  /**
   * Default visibility mode for AI context META_ONLY - only metadata, no values
   * REDACTED - metadata + redacted values FULL - metadata + full values (requires
   * explicit permission)
   */
  private AiVisibilityMode defaultVisibility = AiVisibilityMode.META_ONLY;

  /**
   * Fields that should be redacted (patterns supported) Examples: ["email",
   * "phone", "nationalId", "secret*"]
   */
