 ...
 * 
 * PostgreSQL triggers send NOTIFY on entity changes: - INSERT: CREATE event -
 * UPDATE: UPDATE event - DELETE: DELETE event
 * 
 * This listener invalidates Redis cache keys on changes.
 * 
 * Cache key format: entity:{type}:{id}:{version}
 * 
 * Integration: 1. PostgreSQL trigger sends: NOTIFY change_events,
 * '{"type":"UPDATE","table":"entity","id":"uuid"}' 2. This listener receives
 * event 3. Invalidates Redis cache key 4. Next API call fetches fresh data from
 * DB
 */
...
public class CacheInvalidationListener implements MessageListener {
   ...
   */
  private void invalidateEntity(String entityType, String entityId, String tenantId,
      String eventType) {
    ...
    if (keys != null && !keys.isEmpty()) {
      ...
      log.info("Invalidated cache for entity: type={}, id={}, keys={}, event={}", entityType,
          entityId, keys.size(), eventType);
    }
