 ...
 * 
 * Supported operators: - eq: equals (age eq 18) - ne: not equals (status ne
 * 'inactive') - lt: less than (age lt 30) - lte: less than or equal (age lte
 * 30) - gt: greater than (age gt 18) - gte: greater than or equal (age gte 18)
 * - like: pattern match (name like '%John%') - in: in list (status in
 * ('active', 'pending')) - notIn: not in list (status notIn ('deleted'))
 * 
 * Boolean operators: and, or Parentheses: (age gt 18) and (status eq 'active')
 * 
 * Examples: - status eq 'active' - (age gt 18) and (status eq 'active') - name
 * like '%Smith%' or email like '%@example.com' - status in ('active',
 * 'pending', 'approved') - (age gte 18 and age lte 65) and status ne 'inactive'
 */
@Slf4j
public class FilterParser {
  ...
  private static final Pattern COMPARISON_PATTERN = Pattern
      .compile("(\\w+)\\s+(eq|ne|lt|lte|gt|gte|like)\\s+('[^']*'|\\d+|true|false)");

  private static final Pattern IN_PATTERN = Pattern
      .compile("(\\w+)\\s+(in|notIn)\\s+\\(([^)]+)\\)");
  ...
  private static Condition parseCondition(String expr) {
    ...
    if (inMatcher.matches()) {
      ...
      // Convert values to inline values for jOOQ
      Object[] inlineValues = valueList.stream().map(DSL::inline).toArray();

      if ("in".equals(operator)) {
        return fieldRef.in(inlineValues);
      } else {
        return fieldRef.notIn(inlineValues);
      }
    }
  ...
  private static Condition createComparison(String fieldName, String operator, Object value) {
    ...
    // Use DSL.inline() to create typed values that jOOQ can handle
    ...
    case "like" -> field.like(DSL.inline(value.toString()));
