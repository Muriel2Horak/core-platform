...
public class SearchController {
   ...
   * 
   * Example: GET
   * /api/search?q=john&types=UserProfile,Document&limit=20&minScore=0.1
   */
  @GetMapping
  public ResponseEntity<SearchModels.SearchResponse> search(@RequestParam String q,
      @RequestParam(required = false) String types, @RequestParam(defaultValue = "20") int limit,
    ...
    SearchModels.SearchRequest request = SearchModels.SearchRequest.builder().query(q)
        .entityTypes(entityTypes).limit(limit).minScore(minScore).build();
