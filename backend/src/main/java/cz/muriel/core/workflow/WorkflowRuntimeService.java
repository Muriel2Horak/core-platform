...
public class WorkflowRuntimeService {
  ...
  /**
   * Get workflow graph for entity
   * Returns visual representation with nodes, edges, current state highlighted
   */
  public WorkflowModels.WorkflowGraph getWorkflowGraph(Authentication auth, String entityType, 
    ...
    // Get current state
    WorkflowModels.EntityState currentState = workflowService.getCurrentState(entityType, entityId, tenantId);
    ...
    if (schemaOpt.isEmpty() || schemaOpt.get().getStates() == null || schemaOpt.get().getStates().isEmpty()) {
      log.warn("No workflow states defined for entity type: {}", entityType);
      return WorkflowModels.WorkflowGraph.builder()
    ...
    List<WorkflowModels.GraphNode> nodes = schema.getStates().stream()
        .map(state -> WorkflowModels.GraphNode.builder()
            .id(state.getCode())
            .code(state.getCode())
            .label(state.getLabel() != null ? state.getLabel() : state.getCode())
            .type("state")
    ...
    if (schema.getTransitions() != null) {
      List<WorkflowModels.StateTransition> allowedTransitions = 
          workflowService.getAllowedTransitions(auth, entityType, entityId, tenantId);
      Set<String> allowedCodes = allowedTransitions.stream()
          .map(WorkflowModels.StateTransition::getCode)
      ...
      for (TransitionConfig transition : schema.getTransitions()) {
        ...
        if (!isAllowed) {
          ...
          } else if (transition.getFrom() != null && !transition.getFrom().equals(currentStateCode)) {
        ...
        edges.add(WorkflowModels.GraphEdge.builder()
            .id(transition.getCode())
            ...
            .transitionCode(transition.getCode())
            .allowed(isAllowed)
            .whyNot(whyNot)
            .slaMinutes(transition.getSlaMinutes())
            .build());
      }
    }
  ...
  public WorkflowModels.WorkflowStateDetail getStateDetail(Authentication auth, String entityType,
    ...
    Optional<EntitySchema> schemaOpt = metamodelRegistry.getSchema(entityType);
    List<TransitionConfig> allTransitions = schemaOpt.isPresent() && schemaOpt.get().getTransitions() != null 
        ? schemaOpt.get().getTransitions() 
    ...
    List<WorkflowModels.AllowedTransition> allowed = allowedTransitions.stream()
        .map(t -> WorkflowModels.AllowedTransition.builder()
            .code(t.getCode())
            .label(findTransitionLabel(allTransitions, t.getCode()))
            .toState(t.getToCode())
    ...
    List<WorkflowModels.BlockedTransition> blocked = allTransitions.stream()
        ...
        .filter(t -> t.getFrom() == null || t.getFrom().equals(currentCode)) // Only show relevant blocked ones
        .map(t -> {
          String reason = determineBlockReason(t, currentCode, auth);
          return WorkflowModels.BlockedTransition.builder()
              .code(t.getCode())
              .label(t.getLabel() != null ? t.getLabel() : t.getCode())
              .toState(t.getTo())
              .reason(reason)
              .build();
        })
    ...
    if (currentState != null) {
      ...
      List<Integer> slaMinutes = jdbcTemplate.query(slaSql, 
          (rs, rowNum) -> rs.getObject("sla_minutes", Integer.class),
    ...
    return WorkflowModels.WorkflowStateDetail.builder()
   ...
   */
  public WorkflowModels.WorkflowHistory getHistory(String entityType, String entityId, String tenantId) {
    ...
    List<WorkflowModels.HistoryEntry> entries = jdbcTemplate.query(sql, (rs, rowNum) -> {
      ...
      return WorkflowModels.HistoryEntry.builder()
          ...
          .fromState(rs.getString("from_code"))
          .toState(rs.getString("to_code"))
          .transitionCode(rs.getString("transition_code"))
          .timestamp(timestamp)
    ...
    return WorkflowModels.WorkflowHistory.builder()
        .entityType(entityType)
        .entityId(entityId)
        .entries(entries)
  ...
  public WorkflowModels.WorkflowForecast getForecast(Authentication auth, String entityType,
    ...
    List<WorkflowModels.StateTransition> transitions = 
        workflowService.getAllowedTransitions(auth, entityType, entityId, tenantId);
    ...
    List<WorkflowModels.ForecastStep> nextSteps = transitions.stream()
        .map(t -> WorkflowModels.ForecastStep.builder()
            .transitionCode(t.getCode())
            .label(findTransitionLabel(allTransitions, t.getCode()))
            .toState(t.getToCode())
            .automatic(false) // TODO: determine from step configuration in W7
            .estimatedSlaMinutes(t.getSlaMinutes())
            .build())
        .collect(Collectors.toList());
    ...
    List<WorkflowModels.PendingTimer> pendingTimers = jdbcTemplate.query(timerSql, (rs, rowNum) -> {
      ...
      return WorkflowModels.PendingTimer.builder()
    ...
    return WorkflowModels.WorkflowForecast.builder()
        .currentState(currentStateCode)
        .nextSteps(nextSteps)
  ...
  private String determineBlockReason(TransitionConfig transition, String currentState, Authentication auth) {
    if (currentState == null) {
