 ...
 * 
 * Tracks workflow metrics using Micrometer:
 * - State transition durations
 * - Transition error rates
 * - Number of transitions per entity type
 * 
 * @since 2025-10-14
 */
@Service
...
public class WorkflowMetricsService {
  ...
  public void recordTransitionDuration(String entityType, String fromState, String toState, 
    ...
    Timer.builder("workflow.transition.duration")
        .description("Duration of workflow state transitions")
        .tag("entity_type", entityType)
        .tag("from_state", fromState != null ? fromState : "START")
        .tag("to_state", toState)
        .register(meterRegistry)
        .record(duration);
    
    log.debug("Recorded transition duration: {} -> {} = {}ms", fromState, toState, duration.toMillis());
  }
  ...
  public void recordStateDuration(String entityType, String stateCode, Duration duration) {
    Timer.builder("workflow.state.duration")
        .description("Duration spent in a specific workflow state")
        .tag("entity_type", entityType)
        .tag("state", stateCode)
        .register(meterRegistry)
        .record(duration);
    
    log.debug("Recorded state duration: {} in {} = {}ms", stateCode, entityType, duration.toMillis());
  }
  ...
  public void incrementTransitionCount(String entityType, String transitionCode, boolean success) {
    Counter.builder("workflow.transition.count")
        .description("Number of workflow transitions")
        .tag("entity_type", entityType)
        .tag("transition", transitionCode)
  ...
  public void recordTransitionError(String entityType, String transitionCode, String errorType) {
    Counter.builder("workflow.transition.error")
  ...
  public void recordSlaBreach(String entityType, String stateCode) {
    Counter.builder("workflow.sla.breach")
        .description("SLA breaches in workflow states")
        .tag("entity_type", entityType)
        .tag("state", stateCode)
        .register(meterRegistry)
        .increment();
  ...
  public void recordActiveWorkflows(String entityType, int count) {
    meterRegistry.gauge("workflow.active.count", 
        java.util.List.of(
            io.micrometer.core.instrument.Tag.of("entity_type", entityType)
        ), 
        count);
  }
  ...
  public void recordWorkflowCompletion(String entityType, String status, Duration totalDuration) {
    Counter.builder("workflow.completion")
    ...
    Timer.builder("workflow.total.duration")
    ...
    log.info("Recorded workflow completion: {} / {} = {}ms", entityType, status, totalDuration.toMillis());
  }
}
