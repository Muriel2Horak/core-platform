...
public class WorkflowService {
     ...
     */
    public WorkflowModels.EntityState getCurrentState(String entityType, String entityId, String tenantId) {
        String sql = "SELECT * FROM entity_state WHERE entity_type = ? AND entity_id = ? AND tenant_id = ?";
        
        List<WorkflowModels.EntityState> states = jdbcTemplate.query(sql, 
            new EntityStateRowMapper(), entityType, entityId, tenantId);
     ...
     */
    public List<WorkflowModels.StateTransition> getAllowedTransitions(
        Authentication auth,
        String entityType,
        String entityId,
        String tenantId
        ...
        return transitions.stream()
            .filter(t -> evaluateGuard(auth, t.getGuard()))
            .toList();
    }
     ...
     */
    @Transactional
    public WorkflowModels.TransitionResult applyTransition(
        Authentication auth,
        String entityType,
        String entityId,
        String tenantId,
        ...
        if (transitions.isEmpty()) {
            return WorkflowModels.TransitionResult.builder()
                .success(false)
                .message("Transition not found: " + transitionCode)
                .build();
        }
        ...
        if (!evaluateGuard(auth, transition.getGuard())) {
            return WorkflowModels.TransitionResult.builder()
                .success(false)
                .message("Transition not allowed: guard condition failed")
                .build();
        }
        ...
        if ((transition.getFromCode() == null && fromCode != null) ||
            ...
            return WorkflowModels.TransitionResult.builder()
                .success(false)
                .message("Invalid transition: current state is " + fromCode)
                .build();
        }
        ...
        WorkflowModels.EntityState newState = WorkflowModels.EntityState.builder()
            ...
            .entityId(entityId)
            .tenantId(tenantId)
            .stateCode(transition.getToCode())
            .since(now)
            .build();
    ...
    private class EntityStateRowMapper implements RowMapper<WorkflowModels.EntityState> {
        ...
        public WorkflowModels.EntityState mapRow(ResultSet rs, int rowNum) throws SQLException {
            return WorkflowModels.EntityState.builder()
                .entityType(rs.getString("entity_type"))
                .entityId(rs.getString("entity_id"))
                .tenantId(rs.getString("tenant_id"))
                .stateCode(rs.getString("state_code"))
                .since(rs.getTimestamp("since").toInstant())
                .build();
        }
    }
    ...
    private class StateTransitionRowMapper implements RowMapper<WorkflowModels.StateTransition> {
        ...
        public WorkflowModels.StateTransition mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            if (guardJson != null) {
                try {
                    guard = objectMapper.readValue(guardJson, Map.class);
                } catch (Exception e) {
                    log.warn("Failed to parse guard JSON: {}", guardJson, e);
                }
            }
            
            return WorkflowModels.StateTransition.builder()
                .entityType(rs.getString("entity_type"))
                .fromCode(rs.getString("from_code"))
                .toCode(rs.getString("to_code"))
                .code(rs.getString("code"))
                .guard(guard)
                .slaMinutes((Integer) rs.getObject("sla_minutes"))
                .build();
        }
    }
}
