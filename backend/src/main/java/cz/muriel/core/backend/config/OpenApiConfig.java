 ...
 *
 * <p>
 * Security: - JWT Bearer authentication required for all endpoints - Obtain JWT
 * from Keycloak: POST /realms/{tenant}/protocol/openid-connect/token
 *
...
public class OpenApiConfig {
  ...
  public OpenAPI customOpenAPI() {
    return new OpenAPI()
        .info(new Info().title("Core Platform API").version(apiVersion).description("""
            ...
            **API Versioning:**
            ...
            """)
            .contact(new Contact().name("Core Platform Team").email("support@core-platform.local")
                .url("https://github.com/Muriel2Horak/core-platform"))
            .license(new License().name("Proprietary").url("#")))
        .servers(List.of(
            new Server().url("http://localhost:8080").description("Local Development Server"),
            new Server().url("https://api." + baseDomain).description("Production API Server")))
