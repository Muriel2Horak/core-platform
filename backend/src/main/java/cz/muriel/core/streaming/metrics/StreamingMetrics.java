...
public class StreamingMetrics {
  ...
  public StreamingMetrics(MeterRegistry meterRegistry,
      CommandQueueRepository commandQueueRepository, OutboxFinalRepository outboxFinalRepository,
  ...
  private void registerGauges() {
    ...
    for (String priority : priorities) {
      meterRegistry.gauge("core.stream.cmd.queue.depth", Tags.of("priority", priority),
          commandQueueRepository, repo -> repo.countPendingByPriority(priority));
    }

    // Outbox unsent total
    meterRegistry.gauge("core.stream.outbox.unsent.total", outboxFinalRepository,
        OutboxFinalRepository::countUnsent);

    // Work state counts
    meterRegistry.gauge("core.stream.workstate.updating", Tags.of("status", "updating"),
        workStateRepository, repo -> repo.countByStatus("updating"));

    meterRegistry.gauge("core.stream.workstate.error", Tags.of("status", "error"),
        workStateRepository, repo -> repo.countByStatus("error"));
  }
  ...
  public void recordWorkerSuccess(String entity, String priority) {
    meterRegistry
        .counter("core.stream.worker.success.total", "entity", entity, "priority", priority)
        .increment();
  }
  ...
  public void recordWorkerError(String entity, String priority, String errorType) {
    meterRegistry.counter("core.stream.worker.error.total", "entity", entity, "priority", priority,
  ...
  public void recordDLQ(String entity, String source) {
    meterRegistry.counter("core.stream.dlq.total", "entity", entity, "source", source // "worker" or
                                                                                      // "dispatcher"
    ).increment();
  }
  ...
  public void recordDispatchSuccess(String entity) {
    meterRegistry.counter("core.stream.dispatch.published.total", "entity", entity).increment();
  }
  ...
  public void recordDispatchError(String entity, String errorType) {
    meterRegistry
        .counter("core.stream.dispatch.error.total", "entity", entity, "error_type", errorType)
        .increment();
  }
  ...
  public void recordQuotaThrottle(String entity, String priority) {
    meterRegistry
        .counter("core.stream.quota.throttle.total", "entity", entity, "priority", priority)
        .increment();
  }
  ...
  public void recordLatency(String entity, long latencyMs) {
    meterRegistry.timer("core.stream.latency.accepted.applied.seconds", "entity", entity)
  ...
  public void recordInflightCurrent(String entity, int count) {
    meterRegistry.gauge("core.stream.inflight.current", Tags.of("entity", entity), count);
  }
  ...
  public void updateMetrics() {
    try {
      ...
      meterRegistry.gauge("core.stream.cmd.queue.dlq", commandQueueRepository.countByStatus("dlq"));
