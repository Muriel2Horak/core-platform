 ...
 * 
 * Consumes events from entity.events.* topics This is an example consumer -
 * implement actual business logic as needed
 */
...
public class EventConsumer {
   ...
   * 
   * In production, you might have different consumers for: - Real-time
   * notifications/webhooks - Analytics/reporting pipelines - Search index updates
   * - Cache invalidation
   */
  @KafkaListener(topics = "#{@streamingConfig.getTopic().getPrefix() + '.entity.events.user'}", groupId = "core-platform-events", concurrency = "2", autoStartup = "${streaming.consumer.auto-start:false}" // Disabled
                                                                                                                                                                                                            // by
                                                                                                                                                                                                            // default
  )
   ...
   */
  @KafkaListener(topics = "#{@streamingConfig.getTopic().getPrefix() + '.entity.inflight.user'}", groupId = "core-platform-inflight", concurrency = "1", autoStartup = "${streaming.consumer.auto-start:false}" // Disabled
                                                                                                                                                                                                                // by
                                                                                                                                                                                                                // default
  )
   ...
   */
  @KafkaListener(topics = "#{@streamingConfig.getTopic().getPrefix() + '.dlq.events'}", groupId = "core-platform-dlq", concurrency = "1", autoStartup = "${streaming.consumer.auto-start:false}")
  public void consumeDLQEvents(ConsumerRecord<String, String> record, Acknowledgment ack) {
    try {
      log.error("🚨 DLQ message received: key={}, payload={}", record.key(), record.value());
