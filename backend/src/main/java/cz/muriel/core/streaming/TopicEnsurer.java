 ...
 * 📊 Topic Ensurer
 * 
 * Ensures Kafka topics exist on application startup based on metamodel
 * configuration. Creates topics for: - entity.inflight.{entity} - pre-event
 * notifications (short retention) - entity.events.{entity} - final events
 * (compacted, long retention) - command.{priority}.{entity} - command queues
 * per priority level - dlq.* - dead letter queues
 */
...
public class TopicEnsurer {
  ...
  public TopicEnsurer(StreamingConfig streamingConfig, MetamodelLoader metamodelLoader,
  ...
  public void ensureTopics() {
    ...
    try {
      ...
      topicsToCreate
          .add(createTopic(topicPrefix + "." + globalConfig.getStreaming().getDlqTopic(), 1, // DLQ
                                                                                             // doesn't
                                                                                             // need
                                                                                             // partitions
              globalConfig.getStreaming().getDefaultReplicationFactor()));
      topicsToCreate
          .add(createTopic(topicPrefix + "." + globalConfig.getStreaming().getOutboxDlqTopic(), 1,
              globalConfig.getStreaming().getDefaultReplicationFactor()));
      ...
      for (EntitySchema schema : schemas.values()) {
        ...
        NewTopic inflightTopic = createTopic(topicPrefix + ".entity.inflight." + entityName,
            partitions, replicationFactor);
        int inflightRetentionMs = (entityConfig != null
            && entityConfig.getInflightRetentionMinutes() != null
                ? entityConfig.getInflightRetentionMinutes()
                : globalConfig.getStreaming().getDefaultInflightRetentionMinutes())
            * 60 * 1000;
        inflightTopic.configs(Map.of("retention.ms", String.valueOf(inflightRetentionMs),
            "cleanup.policy", "delete"));
        topicsToCreate.add(inflightTopic);

        // 2. Events topic (long retention, compacted)
        NewTopic eventsTopic = createTopic(topicPrefix + ".entity.events." + entityName, partitions,
            replicationFactor);
        ...
        eventsTopic.configs(Map.of("retention.ms", String.valueOf(eventRetentionMs),
            "cleanup.policy", "compact,delete", "min.compaction.lag.ms", "3600000" // 1 hour
        ));
        topicsToCreate.add(eventsTopic);

        // 3. Command topics per priority
        int cmdRetentionMs = (entityConfig != null
            && entityConfig.getCommandRetentionHours() != null
                ? entityConfig.getCommandRetentionHours()
                : globalConfig.getStreaming().getDefaultCommandRetentionHours())
            * 3600 * 1000;

        for (String priority : Arrays.asList("critical", "high", "normal", "bulk")) {
          NewTopic commandTopic = createTopic(
              topicPrefix + ".command." + priority + "." + entityName, partitions,
              replicationFactor);
          commandTopic.configs(
              Map.of("retention.ms", String.valueOf(cmdRetentionMs), "cleanup.policy", "delete"));
