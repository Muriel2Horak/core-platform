 ...
 * Automaticky synchronizuje uživatele s monitoring rolemi do Grafany
 * 
 * Klíčové features:
 * - ✅ Zpracovává composite roles (např. CORE_ROLE_ADMIN obsahuje CORE_ROLE_MONITORING)
 * - ✅ Kontroluje existenci uživatele v Grafaně před deaktivací
 * - ✅ Automaticky vytváří/aktualizuje/deaktivuje Grafana účty
 * 
 * Aktivace při CDC eventech:
 * - USER_ROLE_ASSIGNED → kontrola monitoring rolí → create/update Grafana user
 * - USER_ROLE_REMOVED → kontrola monitoring rolí → deactivate Grafana user (pokud existuje)
 */
@Slf4j
@Service
...
public class GrafanaUserSyncService {
     ...
     * 
     * DŮLEŽITÉ: Zpracovává i composite roles!
     * - Pokud uživatel má CORE_ROLE_ADMIN, automaticky má i CORE_ROLE_MONITORING
     * - Pokud uživatel má CORE_TENANT_ADMIN, automaticky má i CORE_ROLE_TENANT_MONITORING
     * 
     * Workflow:
     * 1. Načti VŠECHNY role uživatele z Keycloak (včetně composite)
     * 2. Zkontroluj, jestli JAKÁKOLIV role je monitoring role
     * 3a. Pokud ANO → Vytvoř/aktualizuj Grafana uživatele
     * 3b. Pokud NE → Zkontroluj existenci v Grafaně a deaktivuj (pokud existuje)
     */
    public void handleUserRoleChange(Map<String, Object> event) {
        try {
            ...
            log.info("🔄 Processing role change: {} for user: {} in realm: {}", 
    ...
    private UserRepresentation getUserFromKeycloak(String userId, String realmId) {
        try {
            return keycloakAdminClient.realm(realmId)
                .users()
                .get(userId)
                .toRepresentation();
        } catch (Exception e) {
            log.error("❌ Failed to get user from Keycloak: userId={}, realm={}", userId, realmId, e);
            return null;
        }
    }
     ...
     * 
     * Klíčová metoda pro composite role handling!
     * - Vrací i role z composite rolí (např. CORE_ROLE_ADMIN obsahuje CORE_ROLE_MONITORING)
     */
    private Set<String> getEffectiveUserRoles(String userId, String realmId) {
        try {
            List<RoleRepresentation> effectiveRoles = keycloakAdminClient
                .realm(realmId)
                .users()
                .get(userId)
                .roles()
                .realmLevel()
                .listEffective(); // ← Toto vrací i composite role members!
            ...
            log.error("❌ Failed to get effective roles from Keycloak: userId={}, realm={}", 
                userId, realmId, e);
    ...
    /**
     * 🔍 Zkontroluje, jestli uživatel má JAKOUKOLIV monitoring roli
     * (včetně composite role members)
     * 
     * Monitoring roles:
     * - CORE_ROLE_ADMIN (composite → obsahuje CORE_ROLE_MONITORING)
    ...
    private boolean hasAnyMonitoringRole(Set<String> allRoles) {
        return allRoles.contains(CORE_ROLE_ADMIN) ||
               allRoles.contains(CORE_ROLE_MONITORING) ||
               allRoles.contains(CORE_TENANT_ADMIN) ||
               allRoles.contains(CORE_ROLE_TENANT_MONITORING);
    }

    /**
     * 🎭 Určí Grafana roli na základě Keycloak rolí
     * Priority (sestupně):
     * 1. CORE_ROLE_ADMIN → Admin (plný přístup)
     * 2. CORE_ROLE_MONITORING → Editor (může editovat)
     * 3. CORE_TENANT_ADMIN → Editor (composite obsahující monitoring)
     * 4. CORE_ROLE_TENANT_MONITORING → Viewer (pouze čtení)
     */
    ...
    private boolean userExistsInGrafana(String username) {
        try {
            ResponseEntity<Map[]> response = restTemplate.exchange(
                ...
                HttpMethod.GET,
                new HttpEntity<>(createGrafanaHeaders()),
                Map[].class
            );
    ...
    private void deactivateGrafanaUserIfExists(String username) {
        try {
            ...
            restTemplate.exchange(
                grafanaUrl + "/api/admin/users/" + userId,
                HttpMethod.DELETE,
                new HttpEntity<>(createGrafanaHeaders()),
                String.class
            );
     ...
     */
    private void createOrUpdateGrafanaUser(String username, String email, String name, String grafanaRole) {
        try {
            ...
            try {
                HttpEntity<Map<String, Object>> request = new HttpEntity<>(userData, createGrafanaHeaders());
    ...
    private void updateGrafanaUserRole(Integer userId, String role) {
        try {
            ...
            HttpEntity<Map<String, Object>> request = new HttpEntity<>(roleData, createGrafanaHeaders());
            restTemplate.exchange(
                grafanaUrl + "/api/orgs/1/users/" + userId,
                HttpMethod.PATCH,
                request,
                String.class
            );
    ...
    public void handleUserRoleChange(Map<String, Object> event) {
        try {
            ...
            // TODO STEP 1: Načíst VŠECHNY role uživatele z Keycloak Admin API
            // - Použít Keycloak Admin Client
            // - Zavolat GET /admin/realms/{realm}/users/{userId}/role-mappings/realm
            // - To vrátí i composite role members!
            // Set<String> allUserRoles = getUserRolesFromKeycloak(userId, realmId);

            // TODO STEP 2: Zkontrolovat monitoring role (včetně composite)
            // boolean hasMonitoringAccess = hasAnyMonitoringRole(allUserRoles);

            // TODO STEP 3a: Pokud má monitoring přístup → sync do Grafany
            // if (hasMonitoringAccess) {
            // String grafanaRole = determineGrafanaRole(allUserRoles);
            // createOrUpdateGrafanaUser(username, email, name, grafanaRole);
            // }
            // TODO STEP 3b: Pokud NEMÁ monitoring přístup → zkontroluj existenci a
            // deaktivuj
            // else {
            // deactivateGrafanaUserIfExists(username);
            // }

            log.warn(
                    "⚠️ Grafana sync not yet fully implemented - need Keycloak Admin API integration");
     ...
     * 🔍 Zkontroluje, jestli uživatel existuje v Grafaně
     * 
     * DŮLEŽITÉ: Voláme tohle PŘED deaktivací! - Ušetříme zbytečné API cally pokud
     * uživatel v Grafaně nikdy nebyl - Grafana API: GET
     * /api/users/lookup?loginOrEmail={username}
     * 
     * @return true pokud uživatel existuje, false pokud ne
     */
    private boolean userExistsInGrafana(String username) {
        try {
            // TODO: Implementovat Grafana API call
            // ResponseEntity<Map[]> response = restTemplate.exchange(
            // grafanaUrl + "/api/users/lookup?loginOrEmail=" + username,
            // HttpMethod.GET,
            // new HttpEntity<>(createGrafanaHeaders()),
            ...
            log.debug("🔍 Checking Grafana user existence: {}", username);
            return false; // TODO: Implementovat
     ...
     * ❌ Deaktivuje Grafana uživatele POUZE pokud existuje
     * 
     * Workflow: 1. Zkontroluj existenci v Grafaně (userExistsInGrafana) 2. Pokud
     * existuje → Smaž/deaktivuj 3. Pokud NEEXISTUJE → Nic nedělej (ušetříme API
     * call)
     */
    private void deactivateGrafanaUserIfExists(String username) {
        try {
            ...
            // STEP 2: Uživatel existuje → deaktivuj
            log.info("🗑️ Deactivating Grafana user: {}", username);

            // TODO: Implementovat Grafana DELETE API call
            // restTemplate.exchange(
            // grafanaUrl + "/api/admin/users/{userId}",
            // HttpMethod.DELETE,
            // new HttpEntity<>(createGrafanaHeaders()),
            ...
            log.info("✅ Grafana user deactivated: {}", username);
     ...
     * @param grafanaRole Grafana role (Admin/Editor/Viewer)
     */
    private void createOrUpdateGrafanaUser(String username, String email, String name,
        ...
        try {
            ...
            // TODO: Implementovat Grafana Admin API
            // 1. POST /api/admin/users (create)
            // 2. Pokud 409 Conflict → PATCH /api/orgs/1/users/{userId} (update role)
