 ...
 * 
 * Tests version management: - Version creation and auto-increment - Version
...
class WorkflowVersionServiceTest {
  ...
  void startMigration_shouldCreateMigrationRecord() throws Exception {
    ...
    // When
    Long migrationId = versionService.startMigration(v1Id, v2Id, MigrationStrategy.LAZY, "admin");

    // Then
    assertThat(migrationId).isNotNull();

    String strategy = jdbcTemplate.queryForObject(
        "SELECT migration_strategy FROM workflow_version_migrations WHERE id = ?", String.class,
        migrationId);
