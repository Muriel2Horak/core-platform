 ...
 * 1. Kafka presence lock signals (workflow.locks topic)
 * 2. Frontend read-only mode activation
 * 3. ActionsBar disable/enable based on lock status
 * 
 * Flow:
 * - User A opens workflow → publishes lock signal to Kafka
 * - User B receives lock signal via SSE → ActionsBar becomes read-only
 * - User A closes workflow → publishes unlock signal
 * - User B receives unlock signal → ActionsBar becomes editable again
 * 
 * @since 2025-10-14
 */
...
class PresenceLockIT {
    ...
    void testPresenceLockSignalDisablesActions() throws Exception {
        ...
        String lockEvent = objectMapper.writeValueAsString(new LockEvent(
            entityType,
            entityId,
            "LOCK",
            lockingUser,
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create(baseUrl() + "/api/workflows/" + entityType + "/" + entityId + "/lock-status"))
            .GET()
            .build();
    ...
    void testPresenceUnlockSignalEnablesActions() throws Exception {
        ...
        String unlockEvent = objectMapper.writeValueAsString(new LockEvent(
            entityType,
            entityId,
            "UNLOCK",
            lockingUser,
    ...
    void testMultipleUsersPresenceLock() throws Exception {
        ...
        // User A locks
        String lockA = objectMapper.writeValueAsString(new LockEvent(
            entityType,
            entityId,
            "LOCK",
            "user-a@example.com",
        ...
        String lockB = objectMapper.writeValueAsString(new LockEvent(
            entityType,
            entityId,
            "LOCK",
            "user-b@example.com",
