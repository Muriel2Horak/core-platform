 ...
 * 
 * Tests valid/invalid DSL, cache HIT/MISS, circuit breaker, and header isolation.
 * Uses WireMock to mock Grafana without Docker.
 */
...
class MonitoringQueryIT {
  ...
  void validDSL_shouldReturn200_andAddCorrectHeaders() {
    ...
    wireMockServer.stubFor(post(urlPathEqualTo("/api/ds/query"))
    ...
    ResponseEntity<String> response = restTemplate.postForEntity(
        "/api/monitoring/ds/query", request, String.class);
  ...
  void clientSendsGrafanaHeaders_shouldBeIgnored() {
    // Given: Grafana stub
    wireMockServer.stubFor(post(urlPathEqualTo("/api/ds/query"))
        .willReturn(ok().withBody("{\"results\":{}}")));
  ...
  void grafanaReturns500_shouldReturn502_andOpenCircuitBreaker() {
    ...
    ResponseEntity<String> metricsResponse = restTemplate.getForEntity(
        "/actuator/metrics/resilience4j.circuitbreaker.state", String.class);
  ...
  void grafanaTimeout_shouldReturn503() {
    ...
    wireMockServer.stubFor(post(urlPathEqualTo("/api/ds/query"))
        .willReturn(ok().withFixedDelay(10000)));  // 10s delay, timeout is 5s
