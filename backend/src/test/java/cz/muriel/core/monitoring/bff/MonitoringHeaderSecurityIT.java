...
class MonitoringHeaderSecurityIT {
  ...
  void shouldIsolateTenantsByOrgId(WireMockServer wireMock) throws Exception {
    ...
    String orgIdA = wireMock.getAllServeEvents().get(0).getRequest().getHeader("X-Grafana-Org-Id");
    ...
    String orgIdB = wireMock.getAllServeEvents().get(1).getRequest().getHeader("X-Grafana-Org-Id");
  ...
  void shouldRejectRequestWithoutTenant() throws Exception {
    ...
    mockMvc
        .perform(post("/api/monitoring/ds/query").contentType(MediaType.APPLICATION_JSON)
            .content("{\"queries\":[{\"refId\":\"A\",\"expr\":\"{app=\\\"test\\\"}\"}]}"))
