...
@WebMvcTest(KeycloakWebhookController.class) @TestPropertySource(properties = {
...
class KeycloakWebhookControllerTest {
  ...
  void shouldRejectRequestWithInvalidSecret() throws Exception {
    ...
    // When & Then
    mockMvc
        .perform(post("/internal/keycloak/events").header("X-Webhook-Secret", "wrong-secret")
  ...
  void shouldRejectExternalRequest() throws Exception {
    ...
    mockMvc
        .perform(post("/internal/keycloak/events").header("X-Webhook-Secret", "test-secret")
