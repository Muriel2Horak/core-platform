...
@DisplayName("AdminMetamodelController IT")
class AdminMetamodelControllerIT {
  ...
  @Test
  @DisplayName("POST /api/admin/metamodel/reload - should reload metamodel for PLATFORM_ADMIN")
  ...
  void reloadMetamodel_platformAdmin_success() throws Exception {
    ...
    // When: Trigger reload
    mockMvc.perform(post("/api/admin/metamodel/reload"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value("success"))
        .andExpect(jsonPath("$.message").value("Metamodel reloaded successfully"))
        .andExpect(jsonPath("$.schemaCount").exists())
  ...
  @Test
  @DisplayName("POST /api/admin/metamodel/reload - should reload metamodel for OPS")
  ...
  @Test
  @DisplayName("POST /api/admin/metamodel/reload - should deny access for TENANT_ADMIN")
  ...
  @Test
  @DisplayName("POST /api/admin/metamodel/reload - should deny access for regular user")
  ...
  @Test
  @DisplayName("GET /api/admin/metamodel/status - should return status for authenticated user")
  @WithMockUser
  void getMetamodelStatus_authenticated_success() throws Exception {
    mockMvc.perform(get("/api/admin/metamodel/status"))
        .andExpect(status().isOk())
  ...
  @Test
  @DisplayName("GET /api/admin/metamodel/status - should deny access for unauthenticated user")
  ...
  @Test
  @DisplayName("Reload should preserve schema integrity")
  @WithMockUser(roles = "PLATFORM_ADMIN")
