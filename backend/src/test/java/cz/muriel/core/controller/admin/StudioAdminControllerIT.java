...
class StudioAdminControllerIT {
   ...
   */
  @Test
  @WithMockUser(authorities = {"CORE_ADMIN_STUDIO"})
  void testExportEntities_Success() throws Exception {
    mockMvc.perform(get("/api/admin/studio/entities"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value("success"))
        .andExpect(jsonPath("$.entitiesCount").exists())
   ...
   */
  @Test
  @WithMockUser(authorities = {"CORE_ROLE_USER"})
  ...
  void testGetEntity_Success() throws Exception {
    mockMvc.perform(get("/api/admin/studio/entities/User"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.name").value("User"))
        .andExpect(jsonPath("$.entity").value("User"))
        .andExpect(jsonPath("$.table").value("users_directory"))
        .andExpect(jsonPath("$.fields").isArray())
        .andExpect(jsonPath("$.idField").exists())
  ...
  void testHealth() throws Exception {
    mockMvc.perform(get("/api/admin/studio/health"))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.status").value("ok"))
        .andExpect(jsonPath("$.phase").value("S10-B"));
  }
}
