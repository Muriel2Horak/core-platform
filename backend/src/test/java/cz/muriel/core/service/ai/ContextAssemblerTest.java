...
class ContextAssemblerTest {
  ...
  void testAssembleContext_META_ONLY() {
    ...
    Map<String, Object> uiContext = Map.of(
        "screen", Map.of("title", "User Detail", "viewKind", "detail"),
        "fields", List.of(
            Map.of("name", "email", "type", "email", "pii", true)
        ),
        "validations", List.of()
    );
    
    Map<String, Object> wfContext = Map.of(
        "entity", "User",
        "states", List.of(),
        "actions", List.of()
    );
    ...
    for (Map<String, Object> field : fields) {
      assertFalse(field.containsKey("value"), 
          "META_ONLY mode must not contain 'value' field");
      assertFalse(field.containsKey("data"), 
          "META_ONLY mode must not contain 'data' field");
    }
  }
  ...
  void testAssembleContext_ForcesMetaOnly() {
    ...
    when(uiContextService.getCurrentView(routeId)).thenReturn(Map.of(
        "screen", Map.of("title", "User Detail"),
        "fields", List.of(),
        "validations", List.of()
    ));
    ...
    Map<String, Object> policy = (Map<String, Object>) metadata.get("policy");
    assertEquals("META_ONLY", policy.get("visibility"), 
        "Should force META_ONLY for safety");
  }
}
