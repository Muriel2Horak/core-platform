 ...
 * Tests Single-Flight pattern for duplicate query detection.
 */
class QueryDeduplicatorTest {
    ...
    void shouldExecuteQueryOnlyOnceForDuplicates() throws Exception {
        ...
        CompletableFuture<Map<String, Object>> future3 = CompletableFuture.supplyAsync(() ->
            queryDeduplicator.executeWithDeduplication(query, "tenant-1", () -> {
                executionCount.incrementAndGet();
                return expectedResult;
            })
        ...
        assertEquals(1, executionCount.get(), "Query should be executed only once despite 3 concurrent requests");
    ...
    void shouldRemoveFromInflightAfterCompletion() throws Exception {
        ...
        Thread.sleep(50);
        assertTrue(queryDeduplicator.getInflightCount() > 0, "Should have in-flight query during execution");

        // Wait for completion
        future.join();

        // After completion, inflight should be 0
        assertEquals(0, queryDeduplicator.getInflightCount(), "Inflight count should be 0 after completion");
    }
