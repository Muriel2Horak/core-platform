...
class CubeSchemaGeneratorTest {
  ...
  void shouldGenerateBasicCubeSchema() {
    ...
    assertThat(jsCode)
        ...
        .contains("count: {")
        .contains("type: `count`")
        .contains("preAggregations: {")
        .contains("dailyRollup: {")
  ...
  void shouldGeneratePreAggregationsForTimestampFields() {
    ...
    assertThat(jsCode)
        .contains("preAggregations: {")
        .contains("dailyRollup: {")
        .contains("timeDimension: createdAt")
        .contains("granularity: `day`")
        .contains("refreshKey: {")
        .contains("every: `1 hour`");
  }
  ...
  void shouldHandleTenantIsolation() {
    ...
    // Then
    assertThat(jsCode)
        .contains("preAggregationsSchema: `users_preagg_${SECURITY_CONTEXT.tenantId.unsafeValue()}`");
  }
  ...
  void shouldIncludeDrillMembersInCountMeasure() {
    ...
    assertThat(jsCode)
        .contains("count: {")
        .contains("drillMembers: [id, email, name]");
  }
}
