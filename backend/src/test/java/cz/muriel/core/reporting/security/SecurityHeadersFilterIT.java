...
/**
 * Integration tests for SecurityHeadersFilter.
 * Tests OWASP-required security headers enforcement.
 */
@SpringBootTest
...
class SecurityHeadersFilterIT {
    ...
    void shouldIncludeXContentTypeOptionsHeader() throws Exception {
        // Act
        mockMvc.perform(get("/api/reports/metadata/User/spec")
                .header("Authorization", "Bearer test-token"))
    ...
    void shouldApplySecurityHeadersToEntitiesEndpoint() throws Exception {
        ...
        mockMvc.perform(get("/api/entities/User/1")
                .header("Authorization", "Bearer test-token"))
            .andExpect(header().exists("X-Content-Type-Options"))
            .andExpect(header().exists("X-Frame-Options"))
            .andExpect(header().exists("Content-Security-Policy"));
