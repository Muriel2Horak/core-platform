 ...
 * 
 * Tests:
 * - POST bulk-update (should create job and return jobId)
 * - GET job status (should return PENDING → RUNNING → COMPLETED)
 * - POST cancel job (should cancel running job)
 * - Bulk update with >1000 rows (should return 400)
 */
@SpringBootTest
...
class BulkUpdateControllerIT {
    ...
    void shouldCreateBulkUpdateJob() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(post("/api/entities/User/bulk-update")
    ...
    void shouldGetJobStatus() throws Exception {
        ...
        String responseBody = mockMvc.perform(post("/api/entities/User/bulk-update")
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
        ...
        mockMvc.perform(get("/api/bulk-jobs/" + jobId))
    ...
    void shouldReturn404OnNonExistentJob() throws Exception {
        // Act & Assert
        mockMvc.perform(get("/api/bulk-jobs/" + UUID.randomUUID()))
    ...
    void shouldCancelJob() throws Exception {
        ...
        // Act - cancel job
        mockMvc.perform(post("/api/bulk-jobs/" + jobId + "/cancel"))
                .andExpect(status().isNoContent());

        // Assert - job should be cancelled
        mockMvc.perform(get("/api/bulk-jobs/" + jobId))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value(anyOf(is("CANCELLED"), is("PENDING"), is("RUNNING"))));
    }
