...
class BulkUpdateControllerIT extends AbstractIntegrationTest {
    ...
    void shouldCreateBulkUpdateJob() throws Exception {
        ...
        // Act & Assert
        mockMvc.perform(
                post("/api/entities/User/bulk-update").contentType(MediaType.APPLICATION_JSON)
    ...
    void shouldGetJobStatus() throws Exception {
        ...
        String responseBody = mockMvc
                .perform(post("/api/entities/User/bulk-update")
    ...
    void shouldProcessChunks() throws Exception {
        ...
        mockMvc.perform(get("/api/bulk-jobs/" + jobId)).andExpect(status().isOk())
                .andExpect(jsonPath("$.status")
                        .value(anyOf(is("COMPLETED"), is("RUNNING"), is("PENDING"))))
