 ...
 * 
 * Tests workflow runtime API endpoints against real PostgreSQL (Testcontainers):
...
class WorkflowApiIT {
  ...
  @Test
  @WithMockUser(username = "test-user", authorities = {"CORE_ROLE_USER"})
  void testGetWorkflowGraph_withCurrentState() throws Exception {
    // Arrange: Set entity to PENDING state
    insertEntityState("PENDING");

    // Act & Assert
    mockMvc.perform(get("/api/workflows/{entity}/{id}/graph", entityType, entityId)
  ...
  void testGetWorkflowGraph_noCurrentState() throws Exception {
    ...
    mockMvc.perform(get("/api/workflows/{entity}/{id}/graph", entityType, entityId)
            ...
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.currentState").isEmpty())
        .andExpect(jsonPath("$.nodes").isArray());
  }
  ...
  void testGetStateDetail_withAllowedAndBlockedTransitions() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/api/workflows/{entity}/{id}/state", entityType, entityId)
            .header("X-Tenant-Id", tenantId)
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.currentState.stateCode").value("PENDING"))
  ...
  void testGetStateDetail_slaWarning() throws Exception {
    ...
    mockMvc.perform(get("/api/workflows/{entity}/{id}/state", entityType, entityId)
            ...
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.slaStatus").value("WARN")); // 80% threshold exceeded
  }
  ...
  void testGetHistory_withTransitions() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/api/workflows/{entity}/{id}/history", entityType, entityId)
            .header("X-Tenant-Id", tenantId)
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.entityType").value(entityType))
  ...
  void testGetForecast_withPendingTimers() throws Exception {
    ...
    // Act & Assert
    mockMvc.perform(get("/api/workflows/{entity}/{id}/forecast", entityType, entityId)
            .header("X-Tenant-Id", tenantId)
            .accept(MediaType.APPLICATION_JSON))
        .andExpect(status().isOk())
        .andExpect(jsonPath("$.currentState").value("PENDING"))
  ...
  private void insertStateLog(String fromCode, String toCode, String transitionCode, Instant changedAt) {
    jdbcTemplate.update(
  ...
  private void insertPendingTimer(String timerType, Instant scheduledAt, String action) {
    jdbcTemplate.update(
        ...
        UUID.randomUUID(), tenantId, entityType, entityId, timerType, "PENDING", scheduledAt, "PENDING", action
    );
  }
}
