 ...
 * 
 * Tests:
 * - getWorkflowGraph: nodes, edges, current state highlight
 * - getStateDetail: allowed/blocked transitions with "why not" reasons
 * - getHistory: timeline with durations
 * - getForecast: next steps and pending timers
 * 
 * @since 2025-10-14
 */
...
class WorkflowRuntimeServiceTest {
  ...
  void testGetWorkflowGraph_withCurrentState() {
    ...
    when(workflowService.getAllowedTransitions(any(), eq(entityType), eq(entityId), eq(tenantId)))
        .thenReturn(List.of(
            WorkflowModels.StateTransition.builder().code("ship").toCode("SHIPPED").build()
        ));

    // Act
    WorkflowModels.WorkflowGraph graph = runtimeService.getWorkflowGraph(auth, entityType, entityId, tenantId);
    ...
    assertTrue(graph.getNodes().stream().anyMatch(n -> n.getCode().equals("APPROVED") && n.isCurrent()));
    ...
    Optional<WorkflowModels.GraphEdge> shipEdge = graph.getEdges().stream()
        .filter(e -> e.getTransitionCode().equals("ship"))
        .findFirst();
  ...
  void testGetWorkflowGraph_noMetamodel() {
    ...
    // Act
    WorkflowModels.WorkflowGraph graph = runtimeService.getWorkflowGraph(auth, "UnknownEntity", "id", "tenant");
  ...
  void testGetStateDetail_allowedAndBlockedTransitions() {
    ...
    when(workflowService.getAllowedTransitions(any(), eq(entityType), eq(entityId), eq(tenantId)))
        .thenReturn(List.of(
            WorkflowModels.StateTransition.builder()
                .code("ship")
                .toCode("SHIPPED")
                .slaMinutes(60)
                .build()
    ...
    // Act
    WorkflowModels.WorkflowStateDetail detail = runtimeService.getStateDetail(auth, entityType, entityId, tenantId);
  ...
  void testGetStateDetail_slaWarning() {
    ...
    WorkflowModels.EntityState currentState = WorkflowModels.EntityState.builder()
        .stateCode("APPROVED")
        .since(Instant.now().minusSeconds(50 * 60))
        .build();

    when(workflowService.getCurrentState(entityType, entityId, tenantId)).thenReturn(currentState);
    when(metamodelRegistry.getSchema(entityType)).thenReturn(Optional.of(createMockSchema()));
    when(workflowService.getAllowedTransitions(any(), any(), any(), any())).thenReturn(Collections.emptyList());
  ...
  void testGetHistory_withTimeline() {
    ...
    // Act
    WorkflowModels.WorkflowHistory history = runtimeService.getHistory(entityType, entityId, tenantId);
  ...
  void testGetForecast_withPendingTimers() {
    ...
    // Act
    WorkflowModels.WorkflowForecast forecast = runtimeService.getForecast(auth, entityType, entityId, tenantId);
  ...
  private TransitionConfig createTransition(String code, String from, String to, String label, Integer slaMinutes) {
