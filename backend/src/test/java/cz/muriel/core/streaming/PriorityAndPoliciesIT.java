 ...
 * 
 * Tests: - Priority lanes: HIGH priority processes before BULK - Strict reads:
 * GET with ?strict=true returns 423 when entity UPDATING - Status API
...
public class PriorityAndPoliciesIT {
  ...
  void testPriorityLanesProcessingOrder() throws Exception {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      jdbcTemplate.update(
          "INSERT INTO command_queue (id, tenant_id, entity_type, entity_id, operation, payload, status, priority, created_at) "
              + "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?, ?)",
          id, "test-tenant", "User", UUID.randomUUID().toString(), "UPDATE", "{}", "PENDING",
          "BULK", LocalDateTime.now());
    }
          ...
          while (true) {
            ...
            List<Map<String, Object>> batch = jdbcTemplate
                .queryForList("SELECT id FROM command_queue " + "WHERE status = 'PENDING' "
                    ...
                    + "    WHEN 'NORMAL' THEN 2 " + "    WHEN 'BULK' THEN 3 " + "  END, "
                    + "  created_at " + "LIMIT 50 FOR UPDATE SKIP LOCKED");
    ...
    List<LocalDateTime> highCompletions = highIds.stream().map(completionTimes::get)
        .filter(Objects::nonNull).sorted().toList();

    List<LocalDateTime> bulkCompletions = bulkIds.stream().map(completionTimes::get)
        .filter(Objects::nonNull).sorted().toList();
    ...
    if (!highCompletions.isEmpty() && !bulkCompletions.isEmpty()) {
      ...
      long overlapCount = bulkCompletions.stream().filter(t -> t.isBefore(lastHigh)).count();
  ...
  void testStrictReadsReturn423WhenUpdating() {
    ...
    ResponseEntity<String> response = restTemplate
        .getForEntity("/api/users/" + entityId + "?strict=true", String.class);
  ...
  void testStatusAPILifecycle() {
    ...
    jdbcTemplate.update(
        "INSERT INTO command_queue (id, tenant_id, entity_type, entity_id, operation, payload, status, created_at) "
            + "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?)",
        commandId, "test-tenant", "User", entityId.toString(), "UPDATE", "{}", "ACCEPTED",
        LocalDateTime.now());

    String status1 = jdbcTemplate.queryForObject("SELECT status FROM command_queue WHERE id = ?",
        String.class, commandId);
    assertThat(status1).isEqualTo("ACCEPTED");

    // Stage 2: UPDATING
    jdbcTemplate.update("UPDATE command_queue SET status = 'UPDATING' WHERE id = ?", commandId);

    String status2 = jdbcTemplate.queryForObject("SELECT status FROM command_queue WHERE id = ?",
        String.class, commandId);
    ...
    String status3 = jdbcTemplate.queryForObject("SELECT status FROM command_queue WHERE id = ?",
        String.class, commandId);
  ...
  void testPayloadPolicyPIIRedaction() {
    ...
    jdbcTemplate.update(
        "INSERT INTO command_queue (id, tenant_id, entity_type, entity_id, operation, payload, status, created_at) "
            + "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?)",
        commandId, "test-tenant", "User", UUID.randomUUID().toString(), "CREATE", payloadWithPII,
    ...
    jdbcTemplate.update("UPDATE command_queue SET payload = ?::jsonb WHERE id = ?", redactedPayload,
        commandId);
  ...
  void testPayloadMaxSizeEnforcement() {
    ...
    try {
      jdbcTemplate.update(
          "INSERT INTO command_queue (id, tenant_id, entity_type, entity_id, operation, payload, status, created_at) "
              + "VALUES (?, ?, ?, ?, ?, ?::jsonb, ?, ?)",
          commandId, "test-tenant", "User", UUID.randomUUID().toString(), "CREATE",
          largePayload.toString(), "PENDING", LocalDateTime.now());

      // Then: Verify size constraint (implementation-specific)
      String payload = jdbcTemplate.queryForObject(
          "SELECT pg_column_size(payload) as size FROM command_queue WHERE id = ?", String.class,
          commandId);
