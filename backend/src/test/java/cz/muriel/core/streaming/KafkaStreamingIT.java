 ...
 * 
 * Tests:
 * - Topic configuration (cleanup.policy, retention, segment)
 * - Message ordering with keying ({entity}#{entityId})
 * - Event ordering guarantee for same entity
 */
@SpringBootTest
@Testcontainers
public class KafkaStreamingIT {
    ...
    void testTopicConfigurationFromMetamodel() throws ExecutionException, InterruptedException {
        ...
        var topicConfigs = adminClient.describeConfigs(Collections.singleton(configResource))
            .all().get();
    ...
    void testMessageOrderingWithEntityKeying() throws Exception {
        ...
        producer.send(new ProducerRecord<>(topicName, key1, "{\"event\": \"created\", \"version\": 1}")).get();
        producer.send(new ProducerRecord<>(topicName, key2, "{\"event\": \"created\", \"version\": 1}")).get();
        producer.send(new ProducerRecord<>(topicName, key1, "{\"event\": \"updated\", \"version\": 2}")).get();
        producer.send(new ProducerRecord<>(topicName, key2, "{\"event\": \"updated\", \"version\": 2}")).get();
        producer.send(new ProducerRecord<>(topicName, key1, "{\"event\": \"deleted\", \"version\": 3}")).get();
        ...
        while (System.currentTimeMillis() < endTime && (entity1Events.size() < 3 || entity2Events.size() < 2)) {
    ...
    void testPartitionAssignmentConsistency() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            var metadata = producer.send(
                new ProducerRecord<>(topicName, key, "{\"version\": " + i + "}")
            ).get();
