...
public class MockTestConfig {
  ...
  public GrafanaTenantBindingRepository mockGrafanaTenantBindingRepository() {
    ...
    lenient().when(mock.findByTenantId("core-platform")).thenReturn(Optional.of(corePlatformBinding));
    lenient().when(mock.findByTenantId("test-tenant")).thenReturn(Optional.of(testTenantBinding));
    lenient().when(mock.findByTenantId("test_tenant")).thenReturn(Optional.of(testTenantUnderscoreBinding));
    ...
    lenient().when(mock.findByGrafanaOrgId(3L)).thenReturn(Optional.of(testTenantUnderscoreBinding));
