/**
 * 🚀 StreamingScene - Grafana Scenes Component (Native ESM Integration)
 * 
...
export const StreamingScene = ({
  ...
  const initializeScene = async () => {
    try {
      console.log('[StreamingScene] 🚀 Starting native ESM initialization...');
      setLoading(true);
      setError(null);
      ...
      console.log('[StreamingScene] ✅ grafanaBootData exists');
      ...
      console.log('[StreamingScene] ✅ Scenes root container found');
      ...
      console.log('[StreamingScene] 📦 Loading scene factory...');
      const { createStreamingScene } = await import('../../scenes/scene-factories');
      
      // Create and mount scene using centralized factory
      console.log('[StreamingScene] 🎨 Creating scene...');
      const scene = await createStreamingScene(scenesRoot, {
        timeRange: {
          from: timeRange.from,
          to: timeRange.to,
        },
      ...
      console.log('[StreamingScene] ✅ Scene created and activated:', scene);
      setScene(scene);
      setLoading(false);
      console.log('[StreamingScene] 🎉 Initialization complete!');
    } catch (err) {
      console.error('[StreamingScene] ❌ Initialization failed:', err);
      console.error('[StreamingScene] Error stack:', err.stack);
