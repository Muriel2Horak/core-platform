...
Quality gates enforce EPIC-020 checks for pull requests, nightly builds, and releases. Each gate records a JSON result and the aggregator blocks the workflow if any required gate fails (warning gates report but do not block).

## Gate Matrix

- PR: `unit-tests`, `sast`, `sca`, `secret-scan`, `iac-lint`, `ai-guardrails`, `security-regression`
...
5. If any required gate is missing or fails, the aggregator exits non-zero (warning gates are reported separately).
...
- Optional DAST tuning: `DAST_EXCLUDE_REGEX` (comma-separated regex list), `DAST_AUTH_HEADER` (Authorization header value)
...
- `DAST_EXCLUDE_REGEX` limits scan scope (comma-separated regex list).
- `DAST_AUTH_HEADER` injects an Authorization header for protected routes.
