...
run_check "Backend health" \
  "curl -sk -m $TIMEOUT '$BASE_URL/api/actuator/health' | jq -e '.status == \"UP\"'"

run_check "Frontend homepage" \
  "curl -sk -m $TIMEOUT '$BASE_URL/' -o /dev/null"

run_check "Keycloak OIDC config" \
  "curl -sk -m $TIMEOUT '$BASE_URL/auth/realms/admin/.well-known/openid-configuration' | jq -e '.issuer'"

run_check "Grafana health" \
  "curl -sk -m $TIMEOUT '$GRAFANA_URL/api/health' | jq -e '.database == \"ok\"'"
...
if [[ -n "$OIDC_CLIENT_SECRET" ]]; then
  run_check "Auth flow (client credentials)" \
    "curl -sk -m $TIMEOUT -X POST '$BASE_URL/auth/realms/admin/protocol/openid-connect/token' \
