...
def test_json_reporter():
    ...
    result.assert_equal(
        data["overall"]["percentage"], 80.0, "Overall percentage correct"
    )
    result.assert_equal(
        data["coverage"]["code_paths"]["percentage"], 100.0, "code_paths 100%"
    )
    result.assert_equal(
        data["coverage"]["test_paths"]["percentage"], 0.0, "test_paths 0%"
    )
...
def test_performance():
    ...
    result.assert_true(
        ...
        f"Performance < {target_time}s (actual: {elapsed_time:.3f}s)",
    )
    ...
    result.assert_true(
        elapsed_time < 0.5, f"Epic aggregation < 0.5s (actual: {elapsed_time:.3f}s)"
    )
...
def test_edge_cases():
    ...
    story_file = (
        repo_root
        / "backlog/EPIC-001-backlog-system/stories/CORE-001-markdown-structure/README.md"
    )
    ...
    if story_file.exists():
        try:
            ...
            result.assert_equal(
                len(path_mapping["code_paths"]),
                0,
                "Empty code_paths for story without mapping",
            )
            result.assert_equal(
                len(path_mapping["test_paths"]),
                0,
                "Empty test_paths for story without mapping",
            )
            result.assert_equal(
                len(path_mapping["docs_paths"]),
                0,
                "Empty docs_paths for story without mapping",
            )
            ...
            result.assert_equal(
                validation_results["code_paths"].percentage,
                100.0,
                "Empty mapping = 100% (vacuous)",
            )
