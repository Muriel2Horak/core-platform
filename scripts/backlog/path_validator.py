...
def validate_story(
    story_id: str, output_format: str = "text", repo_root: Path = None
) -> int:
...
def validate_epic(
    ...
    if not story_files:
        print(
            f"❌ Error: Epic '{epic_id}' not found or has no stories", file=sys.stderr
        )
        return 1
    ...
    for story_file in story_files:
        try:
            ...
            print(
                f"⚠️  Warning: Skipping {story_file.parent.name}: {e}", file=sys.stderr
            )
            continue
...
See CORE-006 story for full documentation.
    ...
    group.add_argument(
        "--story", metavar="STORY_ID", help="Validate single story (e.g., CORE-005)"
    )
    group.add_argument(
        "--epic",
        metavar="EPIC_ID",
