...
class PathValidationResult:
    ...
    def _validate_glob(self, original_pattern: str, normalized_path: Path) -> None:
        ...
        if matches:
            ...
            relative_matches = [
                str(Path(m).relative_to(self.repo_root)) for m in matches
            ]
...
def validate_path_mapping(
    path_mapping: Dict[str, List[str]], repo_root: Path = None
...
def get_overall_stats(results: Dict[str, PathValidationResult]) -> Dict:
    ...
    return {"total": total, "exist": exist, "percentage": round(percentage, 1)}
...
if __name__ == "__main__":
    ...
    try:
        ...
        for category, result in results.items():
            ...
            print(
                f"{emoji} {category}: {result.exist_count}/{result.total} ({result.percentage:.0f}%)"
            )
        ...
        print(
            f"📈 Overall: {overall['exist']}/{overall['total']} ({overall['percentage']:.0f}%)"
        )
